\section{Spinor-helicity formalism and color-ordered amplitudes}
...
The second one can be derived from the fact that the spinors used here are two-dimensional objects, so a spinor can always be writen as linear combination of two other independent ones.
%
\\\\
In presence of massless vectors, we can write the corresponding polarization vectors by choosing an arbitrary reference momentum $q$ which is not collinear to the momentum $p_i$ of the vector field:
\begin{equation*}
\begin{split}
...
\end{equation}
where $g'$ is the coupling constant and the $A_n[\ldots]$'s are the partial amplitudes.
...
These are amplitudes which break maximally the helicity conservation, having at least two external legs have helicity different from the rest.
